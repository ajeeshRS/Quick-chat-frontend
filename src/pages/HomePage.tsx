...
function HomePage() {
  ...
  useEffect(() => {
    const handleSendMessage = () => {
      ...
      if (messageInp && socketRef.current && formattedDate && formattedTime) {
        const data = {
          message: messageInp,
  ...
  useEffect(() => {
    if (userData && userData.contacts) {
      const contactEmails = userData.contacts
  ...
  useEffect(() => {
    ...
    if (!socketRef.current) {
      ...
      socketRef.current.on("updateSocket", (data) => {
        ...
        setContactDetails((prevContact: any) =>
          prevContact.map((contact: any) =>
            contact.email === email ? { ...contact, socketId } : contact
          )
        );
      });
  ...
  useEffect(() => {
    if (location.pathname.startsWith("/chat/")) {
      const currentContact: any = contactDetails.find(
        (contact: any) =>
          contact.socketId === id || contact.email === peerData.email
      );
      if (currentContact && currentContact.socketId !== id) {
        navigate(`/chat/${currentContact.socketId}`, {
          state: {
            data: { ...currentContact, socketId: currentContact.socketId },
          },
        });
      }
    }
